...
# ==================== CONFIGURATION ====================
ROLLING_DAYS = int(os.getenv('ROLLING_DAYS', '365'))  # Use 365-day rolling average
MIN_DATA_DAYS = int(os.getenv('MIN_DATA_DAYS', '90'))  # Need 90 days before switching from hardcoded
BLEND_PERIOD = int(os.getenv('BLEND_PERIOD', '90'))  # Blend hardcoded + dynamic for 90 days after MIN_DATA_DAYS

HISTORICAL_FOLDER = "historical_fall_data"
ENABLE_DYNAMIC_FALL = os.getenv('ENABLE_DYNAMIC_FALL', '0').lower() in ('1', 'true', 'yes')
...
def store_daily_etf_data(etf_dataframe):
    ...
    Now includes CATEGORY column for 30+ category coverage
    
    Args:
        etf_dataframe: DataFrame with columns: SYMBOL, UNDERLYING_ASSET, %CHNG, VOLUME, LTP, etc.
    """
    ...
    try:
        ...
        # Add CATEGORY column using etf_categorizer
        try:
            ...
            logging.info("Added CATEGORY column for enhanced tracking")
...
def get_dynamic_average_fall_dict(historical_df=None, use_categories=True):
    """
    Generate a dictionary of category/asset -> average_fall
    Uses 30+ categories for comprehensive coverage (86.5% of all ETFs)
    
    Args:
        historical_df: Historical data DataFrame
        use_categories: If True, use CATEGORY column (30+ categories)
                       If False, use UNDERLYING_ASSET (legacy mode)
    
    Returns:
        dict: {category_or_asset: avg_fall_percentage}
    """
    try:
        ...
        # Use CATEGORY-based calculation (covers 30+ categories)
        if use_categories and 'CATEGORY' in historical_df.columns:
            # Get unique categories (excluding None/NaN)
            unique_categories = historical_df['CATEGORY'].dropna().unique()
            
            for category in unique_categories:
                avg_fall = calculate_dynamic_average_fall_by_category(category, historical_df)
                if avg_fall is not None:
                    avg_fall_dict[category] = avg_fall
            
            logging.info(f"Calculated dynamic average falls for {len(avg_fall_dict)} CATEGORIES (30+ coverage)")
