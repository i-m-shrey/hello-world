...
class MorningHealthChecker:
    ...
    def _verify_finvasia_active_sessions(self, session_manager):
        ...
        for client_id, info in list(getattr(session_manager, 'client_sessions', {}).items()):
            ...
            #   2. Injection or Limits call fails  → fall back to _verify_balance_with_retries
            #   3. account_object missing tokens   → fall back to _verify_balance_with_retries
            verified     = False
            balance      = None
            error        = None
            _used_phase1 = False
            ...
            if not _used_phase1:
                # Fallback: fresh verify (original behaviour — re-login with current password)
                verified, balance, error = self._verify_balance_with_retries(
                    client, client.get('password'), attempts=2
                )
    ...
    def _password_policy(self, full_name: str, uid: str) -> str | None:
        """
        Generate a random policy password: FirstName@XXX
        XXX is a random 3-digit number (100-999) chosen at call time.
        Random avoids triggering Finvasia's "cannot reuse last 3 passwords" policy.
        NOTE: result is NOT deterministic — always save old+new to the passwords/
        CSV backup before applying, as the suffix cannot be regenerated.
        """
        import re, random
    ...
    def _rotate_finvasia_password(self, client: dict, error_message: str) -> dict:
        """
        Attempt to rotate the Finvasia password for a client.

        Strategy:
        1. If old == new (policy password already set) → skip change, just verify balance.
        2. Try to change from old → new using change_password_for_client().
        3. If the change API call fails (e.g. already changed externally), try verifying
           balance with the policy password directly — if that works the password was
           already correct and we just update DB.
        4. On successful change, verify balance with retries.
        5. Only update DB once balance verification confirms the new credentials work.
        6. Return an 'updated_client' dict (with new password) so the caller can
           re-initialise the live session.
        """
        broker_id = client.get('broker_id')
        customer_id = client.get('customer_id')
        # Prefer full_name; if missing from client dict look it up from DB directly
        full_name = client.get('full_name') or ''
        if not full_name:
            try:
                with app.app_context():
                    _u = User.query.filter_by(customer_id=customer_id).first()
                    if _u:
                        full_name = _u.full_name or ''
            except Exception:
                pass
        if not full_name:
            full_name = client.get('username') or ''
        old_password = client.get('password')
        new_password = self._password_policy(full_name, client.get('user_id_broker'))
        ...
        # ── Pre-change password backup (CRITICAL with random suffix) ─────────
        # Because the suffix is random it CANNOT be regenerated after the fact.
        # Save old + new to passwords/ CSV NOW, before any API call is made.
        try:
            import csv as _csv
            _pw_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'passwords')
            os.makedirs(_pw_dir, exist_ok=True)
            _pw_ts   = datetime.utcnow().strftime('%Y%m%d_%H%M%S')
            _pw_path = os.path.join(_pw_dir, f'pwd_backup_{customer_id}_{_pw_ts}.csv')
            with open(_pw_path, 'w', newline='', encoding='utf-8') as _f:
                _w = _csv.writer(_f)
                _w.writerow(['customer_id', 'user_id_broker', 'old_password', 'new_password', 'timestamp'])
                _w.writerow([customer_id, client.get('user_id_broker'), old_password, new_password, _pw_ts])
            print(f"  💾 Password backup saved: {_pw_path}")
        except Exception as _bak_err:
            print(f"  ⚠️  Password backup write failed: {_bak_err} — proceeding anyway")

        # ── DEBUG: show passwords and ask before rotating ─────────────────
        if DEBUG:
            print(f"\n[DEBUG] Password rotation triggered for {customer_id}:")
            print(f"  Broker ID   : {client.get('user_id_broker')}")
            print(f"  Old password: {old_password}")
            print(f"  New password: {new_password}")
            _ans = input(f"  Proceed with password change? [y/n]: ").strip().lower()
            if _ans != 'y':
                print(f"  ⏭️  Password change skipped (DEBUG)")
        ...
        def _save_to_db_and_notify(pw: str):
            """Persist new password in DB and notify the client by email."""
            try:
                with app.app_context():
                    broker = db.session.get(Broker, broker_id)
                    if broker:
                        ...
                        db.session.commit()
                        print(f"  💾 DB updated — new password saved for broker_id={broker_id}")
            except Exception as db_err:
                print(f"  ⚠️ DB update failed for {customer_id}: {db_err}")

            # Get client email — fallback to DB lookup if not in client dict
            ...
            if not client_email:
                try:
                    with app.app_context():
                        _u = User.query.filter_by(customer_id=customer_id).first()
                        if _u:
                            client_email = _u.email
                            print(f"  📧 Fetched email from DB for {customer_id}: {client_email}")
                except Exception as lookup_err:
                    print(f"  ⚠️ Could not fetch email from DB for {customer_id}: {lookup_err}")

            if client_email:
                try:
                    send_finvasia_password_reset_email(
                        client_email,
                        full_name,
                        customer_id,
                        pw
                    ...
                    print(f"  ✅ Client password reset email sent to {client_email}")
                except Exception as email_err:
                    result['client_email'] = client_email
                    result['client_email_sent'] = False
                    print(f"  ❌ Failed to send client email to {client_email}: {email_err}")
            else:
                result['client_email'] = None
                result['client_email_sent'] = False
                print(f"  ⚠️ No email address found for {customer_id} — client not notified")

        # ── Shortcut: old == new → policy password already set, just verify ──
        if old_password == new_password:
            print(f"  ℹ️ {customer_id}: policy password already set, verifying balance...")
            verified, balance, verify_error = self._verify_balance_with_retries(client, new_password)
            ...
            if verified:
                ...
                result['error'] = verify_error or 'balance_verification_failed'
            return result

        # ── Step 1: Attempt password change old → new ─────────────────────────
        # IMPORTANT: verify=False prevents TOTP reuse.
        # /Changepwd consumes a TOTP code. Calling login_with_totp() inside
        # change_password_for_client(verify=True) within the same 30-second TOTP
        # window reuses the same code → Finvasia rejects it → function returns False
        # even though the password was actually changed. We do our own verify below.
        ...
        try:
            with client_proxy_context(get_client_proxy(client)):
                change_success = change_password_for_client(
                    ...
                    totp_fn=_totp,
                    verify=False,   # ← Critical fix: no TOTP reuse, we verify below
                )
        except Exception as change_err:
            print(f"  ⚠️ {customer_id}: change_password_for_client raised: {change_err}")
            result['change_error'] = str(change_err)
            # ── Fallback: maybe password was already rotated to the policy value ──
            print(f"  🔄 {customer_id}: trying balance check with policy password as fallback...")
            verified, balance, verify_error = self._verify_balance_with_retries(client, new_password)
            result['verified'] = verified
            result['balance'] = balance
            result['verify_error'] = verify_error
            if verified:
                _save_to_db_and_notify(new_password)
                client['password'] = new_password
                result['success'] = True
                result['updated_client'] = dict(client)
                print(f"  ✅ {customer_id}: policy password already active — DB updated.")
            else:
                result['error'] = str(change_err)
                print(f"  ❌ {customer_id}: both change and fallback verify failed.")
            return result

        if not change_success:
            # /Changepwd returned Not_Ok with the current old_password.
            # Could mean password was already changed externally to policy value.
            # Try fallback verify before giving up.
            print(f"  🔄 {customer_id}: change_password returned False, trying policy password directly...")
            verified, balance, verify_error = self._verify_balance_with_retries(client, new_password)
            result['verified'] = verified
            result['balance'] = balance
            result['verify_error'] = verify_error
            if verified:
                ...
                result['success'] = True
                result['updated_client'] = dict(client)
                print(f"  ✅ {customer_id}: policy password confirmed already active — DB updated.")
            else:
                result['error'] = 'change_or_verify_failed'
                print(f"  ❌ {customer_id}: change failed and fallback verify also failed.")
            return result

        # ── Step 2: /Changepwd returned Ok → password IS changed at broker. ──────
        # Save to DB IMMEDIATELY — before any verification attempt.
        # If we save only after verify and verify fails for any transient reason
        # (network blip, rate limit, TOTP timing), the new password is lost from
        # our DB. Next run then tries the wrong (old) password forever.
        _save_to_db_and_notify(new_password)
        client['password'] = new_password
        result['updated_client'] = dict(client)
        print(f"  💾 {customer_id}: password changed — DB saved immediately (before verify).")

        # ── Step 3: Verify new credentials by fetching balance ────────────────
        # Sleep briefly so we're guaranteed to be in a fresh TOTP window.
        time.sleep(5)
        verified, balance, verify_error = self._verify_balance_with_retries(client, new_password)
        result['verified'] = verified
        result['balance'] = balance
        ...
        if verified:
            result['success'] = True
            print(f"  ✅ {customer_id}: password rotated and balance verified (₹{balance}).")
        else:
            # Password was changed and DB was saved. A transient verify failure
            # (TOTP timing, network) must NOT mask a successful password change.
            # Mark success=True so the session is re-initialised with the new
            # password — it will work at market open even if verify failed now.
            result['success'] = True
            result['verify_warning'] = verify_error or 'balance_verification_timed_out'
            print(
                f"  ⚠️ {customer_id}: password changed (DB saved) but balance verify failed: {verify_error}\n"
                f"  ℹ️ {customer_id}: marking SUCCESS — new password saved, session will re-login at next run."
            )

        return result

    def _verify_balance_with_retries(self, client: dict, new_password: str, attempts: int = 4):
        """
        Verify that new_password actually works by fetching the available balance.

        Two critical fixes vs the old version:
        1. proxy_ip must be included in the verify-client dict so finvasia_broker_api
           routes through the registered static IP (required for GenAcsTok).
        2. The session cache must be cleared BEFORE each attempt. Without this,
           finvasia_broker_api._get_or_create_session returns the stale pre-change
           session (keyed only by user_id_broker) and completely ignores new_password,
           making the verification meaningless.
        """
        ...
        for attempt in range(attempts):
            # Force a fresh login with new_password on every attempt.
            # Without clearing, _get_or_create_session returns the old cached
            # session and never tests the new password.
            try:
            ...
            try:
                verify_client = {
                    ...
                    "proxy_ip":       proxy_url,   # ← was missing; required for GenAcsTok IP check
                }
                balance = get_available_funds(verify_client)
                return True, balance, None
            except Exception as e:
                last_error = str(e)
                logging.warning(
                    f"  _verify_balance_with_retries attempt {attempt + 1}/{attempts} "
                    f"for {uid}: {last_error}"
                )
                time.sleep(2.0)   # 2 s gap; also helps TOTP window rotation
                ...
                <div style="padding:20px;">
                    <table style="width:100%;border-collapse:collapse;font-size:14px;">
                        ...
                        <tr>
                            ...
                            <td style="padding:8px 0;color:#dc3545;">{result.get('error') or result.get('verify_error') or '—'}</td>
                        </tr>
                    </table>
                </div>
            </div>
