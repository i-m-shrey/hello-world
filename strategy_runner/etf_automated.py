...
def _password_policy(full_name: str, uid: str):
    """
    ...
    NOTE: result is NOT deterministic — always save to the passwords/ CSV backup
    before applying, as it cannot be regenerated later.
    """
    import re, random
...
def _rotate_finvasia_password(client: dict, error_message: str) -> dict:
    """
    Attempt to rotate the Finvasia password for a client during order execution.
    Strategy:
    1. Try to change old -> new using change_password_for_client().
    2. If change API fails (e.g. already changed), try logging in with new policy
       password directly to confirm it's already in use, then update DB.
    3. Returns dict with 'success', 'new_password', 'error' keys.
    """
    broker_id = client.get('broker_id')
    customer_id = client.get('customer_id')
    full_name = client.get('full_name') or client.get('username') or ''
    old_password = client.get('password')
    new_password = _password_policy(full_name, client.get('user_id_broker'))
    result = {'success': False, 'error': None, 'new_password': None}
    if not new_password:
    ...
    try:
        change_ok = change_password_for_client(
            ...
            totp_fn=_totp,
            verify=False,  # Changepwd success = password changed; etf_automated does its own verify
        )
    except Exception as change_err:
        logging.warning(f"[{customer_id}] change_password_for_client raised: {change_err}. Trying policy password as fallback.")
    ...
    try:
        ...
        if broker:
            ...
            broker.last_updated = dt.utcnow()
            db.session.commit()
    except Exception as db_err:
        logging.warning(f"[{customer_id}] DB update failed after password rotation: {db_err}")
    result['success'] = True
    result['new_password'] = new_password
    return result
                    ...
                    if rotation.get('success'):
                        ...
                        try:
                            ...
                            if user and user.email:
                                from email_notifications import send_client_notification_email
                                send_client_notification_email(
                                    user.email,
                                    "Your Finvasia Password Has Been Updated",
                                    f"""Dear {user.full_name or customer_id},

Your Finvasia trading password has been updated automatically because the 90-day broker policy requires a reset.

Your account is fully operational — no action is needed on your part.

Regards,
SmartETF Team"""
                                )
                        except Exception as email_err:
                            logging.warning(f"[{customer_id}] Email send failed: {email_err}")
                    else:
