...
def _intact(path: str) -> bool:
    """Header + parseable last row (epoch-ms timestamp, 6 columns) — rejects
    truncated/partial files left behind by an interrupted or failed fetch."""
    try:
        ...
        with open(path, "rb") as f:
            ...
            lines = [ln for ln in f.read().decode(errors="ignore").splitlines() if ln.strip()]
        if not head.startswith("timestamp") or not lines:
            return False
        last = lines[-1].split(",")
        return len(last) == 6 and last[0].isdigit()
    except OSError:
        return False
...
def fetch(frm: str, to: str, price: str) -> str | None:
    ...
    if os.path.exists(path):
        if _intact(path):
            ...
            return path
        print(f"  stale  {name}: incomplete file, refetching")
        os.remove(path)
    ...
    for attempt in range(1, RETRIES + 1):
        try:
            r = subprocess.run(cmd, cwd=DATA, capture_output=True, text=True,
                               timeout=900)
            if r.returncode == 0 and _intact(path):
                rows = sum(1 for _ in open(path)) - 1
                print(f"  fetch  {name}: {rows} rows (attempt {attempt})")
                return path
            if os.path.exists(path):
                os.remove(path)          # never leave a partial chunk behind
