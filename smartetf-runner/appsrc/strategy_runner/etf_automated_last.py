...
def run_strategy():
    ...
    with app.app_context():
        # Step 1: Identify the master broker
        ...
        master_broker_name = master_broker.broker_name.upper()
        print(f"🔍 Master broker identified: {master_broker_name}")
        ...
        # Step 2: Execute master orders
        try:
            master_executor = get_executor_for_broker(master_broker_name)
            print(f"▶️ Executing master account first...")
            master_executor.place_order(master_info, filtered_etfs)
        except Exception as e:
            print(f"❌ Master execution failed:", e)
            return

        # Step 3: Dispatch copies to all brokers (including master broker's clients)
        print("🔁 Starting copy to all subscribed clients...")
        clients = get_active_clients()
        print(f"Total number of clients: {len(clients)}")
        broker_map = {}

        for client in clients:
            if client.get("is_master"):
                continue
            broker_name = client["broker_name"].upper()
            broker_map.setdefault(broker_name, []).append(client)

        for broker_name, client_group in broker_map.items():
            try:
                executor = get_executor_for_broker(broker_name)
                if broker_name == master_broker_name:
                    print(f"▶️ Copying orders to {broker_name} clients (excluding master)...")
                else:
                    print(f"▶️ Copying orders to {broker_name} clients ({len(client_group)})...")

                for client in client_group:
                    executor.place_order(client, filtered_etfs)
            except Exception as e:
                print(f"⚠️ Error executing for broker '{broker_name}':", e)
