...
def fetch_cookies_with_selenium(headless: bool = True):
    ...
    try:
        ...
        logging.info("Loading NSE homepage...")
        driver.get("https://www.nseindia.com/")
        time.sleep(2)
        
        logging.info("Loading ETF page...")
        ...
        if not cookies:
            ...
            logging.warning("No cookies found, refreshing page...")
            driver.refresh()
            time.sleep(5)
            cookies = {cookie['name']: cookie['value'] for cookie in driver.get_cookies()}
        ...
        if driver:
            try:
                ...
                logging.warning(f"Error closing driver: {quit_err}")
