from app import app
from mstock_account import MStockAccount
from validate_client_sessions import send_email


# def execute_for_mstock(filtered_etfs_df, clients=None):
#     with app.app_context():
#         if not clients:
#             print("❌ No clients provided for MStock execution.")
#             return
#
#         print(f"🚀 Executing MStock copy for {len(clients)} client(s)...")
#
#         for client in clients:
#             if client['broker_name'].upper() != 'MSTOCK':
#                 continue
#
...
#             print(f"🔑 Logging in MStock client: {client['username']}")
#             account = MStockAccount(
...
def place_order(account_info, filtered_etfs_df, is_amo=False):
    ...
    print(f"🚀 Placing {order_type_label} orders for {account_info['username']} via MSTOCK...")
    if is_amo:
        print(f"⚠️ AMO orders not yet implemented for MSTOCK")

    try:
        account = MStockAccount(username=account_info['user_id_broker'], password=account_info['password'], api_key=account_info['api_key'])
        account.login()
...
def test_sessions(clients):
    print(f"🔍 MStock: Testing sessions for {len(clients)} client(s)...")
    for client in clients:
        ...
        try:
            account = MStockAccount(username=client['user_id_broker'], password=client['password'], api_key=client['api_key'])
            account.login()
