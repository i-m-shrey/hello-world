...
class GenericOrderExecutor:
    ...
    def place_single_symbol_order(self, symbol, qty, price, full_etf_df=None):
        ...
        result = self._try_place_order(symbol, qty, ltp=price)
    ...
    def place_all_orders(self, etf_orders_df, full_etf_df=None):
        ...
        for _, row in etf_orders_df.iterrows():
            ...
            if result.get('status') == 'FAILED' and 'Token refresh failed' in result.get('reason', ''):
                token_failed = True
                print(f"    🚨 Token failure detected - will skip remaining {len(etf_orders_df) - len(results)} orders for this client")
        
        return results
    
    def _try_place_order(self, symbol, qty, ltp=None):
        """
        Try to place order using broker API with token retry logic.
        ltp: pre-fetched LTP from ETF CSV — passed to broker API to avoid
             redundant live fetch (critical when market is closed).
        Returns:
            dict: {'success': bool, 'order_id': str, 'error': str}
        """
        try:
            order_id = self.broker_api.place_order(symbol, qty, ltp=ltp)
            return {'success': True, 'order_id': order_id, 'error': None}
        except Exception as e:
            error_str = str(e).lower()
            
            # FINVASIA: place_single_order_direct already handles one internal retry.
            # Extra retries here create 6+ logins per order. Return failure immediately.
            ...
            if is_token_error:
                ...
                try:
                    ...
                    if is_still_token_error:
                        logging.error(f"Token refresh failed - still token error: {retry_error}")
                        print(f"    ❌ Token refresh failed - skipping all remaining orders for this client")
                        return {'success': False, 'order_id': None, 'error': f"Token refresh failed: {retry_error}", 'skip_alternatives': True}
                    else:
