...
MAX_PRICE_TOLERANCE_PERCENT = 40.0  # Allow up to 40% above allocation for 1 qty
...
def calculate_quantities(filtered_etfs):
    ...
    if not zero_qty_etfs.empty:
        ...
        for idx, row in zero_qty_etfs.iterrows():
            ...
            # Prefer the largest fund first (highest volume) if within tolerance
            category_etfs = category_etfs.sort_values(['VOLUME', 'LTP'], ascending=[False, True])
            tolerance = allocated * (1 + MAX_PRICE_TOLERANCE_PERCENT / 100.0)
            top_volume = category_etfs.iloc[0]
            top_price = top_volume['LTP']
            if top_price <= allocated:
                qty = int(allocated / top_price)
                print(
                    f"  ✅ {row['SYMBOL']} (₹{current_price:.2f}) → {top_volume['SYMBOL']} "
                    f"(₹{top_price:.2f}, top volume) | Qty: {qty}"
                )
                filtered_etfs.loc[idx, 'SYMBOL'] = top_volume['SYMBOL']
                filtered_etfs.loc[idx, 'LTP'] = top_price
                filtered_etfs.loc[idx, 'VOLUME'] = top_volume['VOLUME']
                filtered_etfs.loc[idx, '%CHNG'] = top_volume['%CHNG']
                filtered_etfs.loc[idx, 'QTY'] = qty
                filtered_etfs.loc[idx, 'FINAL_AMOUNT'] = qty * top_price
            elif top_price <= tolerance:
                print(
                    f"  ✅ {row['SYMBOL']} (₹{current_price:.2f}) → {top_volume['SYMBOL']} "
                    f"(₹{top_price:.2f}, top volume, within {MAX_PRICE_TOLERANCE_PERCENT:.0f}%) | Qty: 1"
                )
                filtered_etfs.loc[idx, 'SYMBOL'] = top_volume['SYMBOL']
                filtered_etfs.loc[idx, 'LTP'] = top_price
                filtered_etfs.loc[idx, 'VOLUME'] = top_volume['VOLUME']
                ...
                filtered_etfs.loc[idx, 'FINAL_AMOUNT'] = top_price
            else:
                # Sort by price ascending, volume descending for fallback selection
