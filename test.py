...
def main() -> int:
    parser = argparse.ArgumentParser(description="Backtest Finvasia password rotation (non-Selenium).")
    parser.add_argument("--db-url", default=DEFAULT_DB_URL, help="Database URL (defaults to DATABASE_URL env var)")
    parser.add_argument("--apply", action="store_true", help="Actually rotate passwords (default is dry-run)")
    parser.add_argument("--verify", action="store_true", help="Verify login after password change")
    parser.add_argument("--verify-balance", action="store_true", help="Fetch balance after rotation to verify login")
    parser.add_argument("--customer-id", help="Run for a single customer_id (e.g., smartetf_user_10001)")
